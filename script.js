...
// ==========================================
// 🚨 O'ZGARTIRING: Botingiz Usernamesi
...
function buySpecialOffer() {
    // Web App yopiladi va botga /start buy_special buyrug'i ketadi
    tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=buy_special`);
    tg.close();
}

function buyRealMoney() {
    // Web App yopiladi va botga /start buy_coins buyrug'i ketadi
    tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=buy_coins`);
    tg.close();
}

// ================== AKSIYA VA BOZOR ==================
function openSpecialOffer() { document.getElementById("special-offer-modal").style.display = "flex"; }
function closeSpecialOffer() { document.getElementById("special-offer-modal").style.display = "none"; }
setTimeout(() => { if (game.level < 5) openSpecialOffer(); }, 1500);
...
function parseWarehouse(str) {
    ...
    if (!str) return wh;
    str.split(',').forEach(p => { if(p.includes(':')) { let [k,v]=p.split(':'); let key = (k=='E')?'eggs':(k=='W'?'wool':'meat'); wh[key]=parseInt(v); }});
    return wh;
}
...
function renderMarketList() {
    ...
    list.innerHTML = "";
    if (game.marketData.length === 0) { list.innerHTML = "<p style='text-align:center; color:#ddd; padding: 20px;'>Bozor bo'sh...</p>"; return; }
    game.marketData.forEach(item => {
        let div = document.createElement('div');
        div.className = 'market-item';
        let emoji = (item.type=='eggs')?'🥚':(item.type=='wool'?'🧶':'🥩');
        div.innerHTML = `<div class="m-info"><span style="font-size:12px; color:#555;">Sotuvchi: <b>${item.seller}</b></span><br><span style="font-size:16px;">${emoji} <b>${item.qty}</b> dona</span></div><div style="text-align:right;"><div class="m-price">${item.price} FAM</div><button class="btn-buy-market" onclick="buyFromMarket(${item.id})">OLISH</button></div>`;
...
function sellToMarket() {
    ...
    if ((game.warehouse[item] || 0) < qty) { tg.showAlert("Omborda yetarlicha mahsulot yo'q!"); return; }
    tg.sendData(JSON.stringify({ action: "sell_market", item: item, qty: qty, price: price })); tg.close();
}
function buyFromMarket(id) {
    tg.showConfirm("Haqiqatan ham sotib olasizmi?", (c) => { if(c) { tg.sendData(JSON.stringify({ action: "buy_market", market_id: id })); tg.close(); } });
}
...
function renderInterior(type) {
    ...
    if (count === 0) { grid.innerHTML = "<p style='grid-column: span 3; color: #ffe0b2; text-align: center; font-size: 18px;'>Bu yer bo'm-bo'sh...</p>"; return; }
    for (let i = 0; i < count; i++) { createAnimalCard(type, i, grid); }
}
    ...
    animalTimers.forEach(animal => {
        ...
        if (animal.progress >= 100) {
            ...
            if (game.food >= cost) {
                ...
                if(game.xp >= game.level*100) { game.xp=0; game.level++; tg.showAlert(`Level Up!`); }
                updateUI();
...
function buyAnimal(type, price) {
    if (game.level < CONFIG[type].levelReq) { tg.showAlert(`Lvl ${CONFIG[type].levelReq} kerak!`); return; }
    if (game.balance >= price) {
        game.balance -= price;
        game.inventory[type] = (game.inventory[type] || 0) + 1;
        updateUI();
        if (currentView === 'building') { renderInterior(type); } else { tg.showAlert(`Sotib olindi!`); }
    } else { tg.showAlert("Pul yetmaydi!"); }
}
function buyFood() {
    if (game.balance >= 2) { game.balance -= 2; game.food += 5; updateUI(); } else tg.showAlert("Pul yetmaydi!");
}
function sellAll() {
...
function saveGame() {
    ...
    tg.sendData(JSON.stringify({ balance: game.balance, inventory: invStr, warehouse: warStr, food: game.food, level: game.level, xp: game.xp }));
}
updateUI();